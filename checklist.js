...
(() => {
  ...
  Object.assign(COPY, {
    headline:['તમારી ઉજવણી. એક ટૂંકી યાદી.','Your celebration. A simple shortlist.'],
    heroText:['પ્રસંગો પસંદ કરો, જરૂરી સેવાઓ જણાવો. બાકીની વિગતો સાથે નક્કી કરીશું.','Choose your events and the help you need. We’ll work out the details together.'],
    heroFoot:['પ્રસંગો · સેવાઓ · ખાસ નોંધ','EVENTS · SERVICES · A LITTLE ABOUT YOU'],
    quickView:['ટૂંકું ફોર્મ','Quick brief'],detailedView:['વિગતવાર ચેકલિસ્ટ','Detailed checklist'],
    quickTitle:['કયા પ્રસંગોનું આયોજન છે?','What are you celebrating?'],
    quickHint:['બધા પ્રસંગો અહીં જ પસંદ કરો. તારીખ કે વિગતો નક્કી ન હોય તો ખાલી રાખો.','Choose your events here. Leave anything undecided blank.'],
    yourEvents:['તમારા પ્રસંગો','Your events'],extraEvents:['બીજી વ્યવસ્થાઓ','Other arrangements'],
    serviceTitle:['કઈ સેવાઓ જોઈએ છે?','What would you like help with?'],
    serviceHint:['બધા પસંદ કરેલા પ્રસંગો માટે એક જ વાર પસંદ કરો. ખાસ જરૂરિયાત નીચે લખો.','Select once for all your chosen events. Add any event-specific requests below.'],
    decor:['સજાવટ અને ફૂલો','Decor & flowers'],stage:['સ્ટેજ અને બેઠક','Stage & seating'],
    sound:['લાઇટ અને સાઉન્ડ','Lighting & sound'],photo:['ફોટો અને વીડિયો','Photo & video'],
    food:['જમવાની વ્યવસ્થા','Food & catering'],guestHelp:['મહેમાનોની વ્યવસ્થા','Guest hospitality'],
    entertainment:['સંગીત અને મનોરંજન','Music & entertainment'],rituals:['વિધિની વ્યવસ્થા','Ritual arrangements'],
    sharedServices:['બધા પ્રસંગો માટે જરૂરી સેવાઓ','Services across your events'],
    quickNotes:['કોઈ ખાસ જરૂરિયાત?','Anything else we should know?'],
    quickPlaceholder:['જેમ કે હલદીમાં પીળાં ફૂલો, રિસેપ્શનમાં ૩૦૦ મહેમાનો…','For example: yellow flowers for Haldi, 300 guests at the reception…'],
    moreDetails:['વધુ વિગતો ઉમેરો (વૈકલ્પિક)','Add more details (optional)'],
    quickClient:['તમારી વિગતો','A little about you'],
    eventsCount:['પસંદ કરેલા પ્રસંગો','events chosen'],servicesCount:['પસંદ કરેલી સેવાઓ','services chosen'],
    editBrief:['ફોર્મમાં પાછા જાઓ','Back to your brief'],
    quickEmpty:['પ્રસંગો કે સેવાઓ પસંદ કરો અથવા તમારી વિગતો ઉમેરો.','Choose events or services, or add your details.'],
    quickStart:['પ્રસંગો પસંદ કરો','Choose your events'],
    selectionSummary:['પ્રસંગો અને સેવાઓ પસંદ કર્યાં','events and services chosen'],
    detailedSaved:['વિગતવાર ચેકલિસ્ટની પસંદગી પણ સારાંશમાં સામેલ છે.','Your detailed checklist choices are included in the brief too.']
  });
  ...
  const fresh = () => ({version:1,lang:'gu',view:'quick',services:[],client:{},sections:{},items:{}});
  ...
  function sanitize(raw) {
    ...
    value.view = raw.view === 'detailed' ? 'detailed' : 'quick';
  ...
  function renderClient() {
    ...
    const basic = ['clientName','phone','date','city'];
    $('#client-fields').innerHTML = state.view==='quick'
      ? basic.map(render).join('') + `<details class="more-client"><summary>${esc(t('moreDetails'))}</summary><div class="field-grid">${CLIENT_FIELDS.filter(k=>!basic.includes(k)&&k!=='preferences').map(render).join('')}</div></details>`
      : CLIENT_FIELDS.map(render).join('');
    $('#client-details').hidden = reviewing;
    $('#client-details [data-t="clientDetails"]').textContent=t(state.view==='quick'?'quickClient':'clientDetails');
    if(state.view==='quick') $('#client-details').open=true;
  }
  function chosenEvents() { return DATA.filter(relevant); }
  function updateQuickCount() {
    const counter=$('#quick-counter');
    if(counter) counter.textContent=`${chosenEvents().length} ${t('eventsCount')} · ${state.services.length} ${t('servicesCount')}`;
  }
  function renderQuick() {
    const events = sections => sections.map(s=>`<label class="quick-choice"><input type="checkbox" data-event="${s.id}"${relevant(s)?' checked':''}><span>${esc(tr(s.title))}</span></label>`).join('');
    $('#step-content').innerHTML=`<section class="quick-section"><div class="quick-heading"><span class="eyebrow">01 / ${esc(t('yourEvents'))}</span><h2 id="step-title" tabindex="-1">${esc(t('quickTitle'))}</h2><p>${esc(t('quickHint'))}</p></div><div class="quick-choices" role="group" aria-labelledby="step-title">${events(DATA.filter(s=>!s.optional&&s.id!=='essentials'))}</div><details class="extra-events"${DATA.some(s=>(s.optional||s.id==='essentials')&&relevant(s))?' open':''}><summary>${esc(t('extraEvents'))} <span>(${esc(t('optional'))})</span></summary><div class="quick-choices">${events(DATA.filter(s=>s.optional||s.id==='essentials'))}</div></details></section><section class="quick-section"><div class="quick-heading"><span class="eyebrow">02 / ${esc(t('sharedServices'))}</span><h2 id="services-title">${esc(t('serviceTitle'))}</h2><p>${esc(t('serviceHint'))}</p></div><div class="quick-choices" role="group" aria-labelledby="services-title">${SERVICES.map(id=>`<label class="quick-choice"><input type="checkbox" data-service="${id}"${state.services.includes(id)?' checked':''}><span>${esc(t(id))}</span></label>`).join('')}</div><label class="quick-notes" for="quick-notes">${esc(t('quickNotes'))}<textarea id="quick-notes" data-client="preferences" maxlength="2000" placeholder="${esc(t('quickPlaceholder'))}">${esc(state.client.preferences)}</textarea></label>${allSelected().length?`<p class="hint retained-choices">${esc(t('detailedSaved'))}</p>`:''}</section><div class="quick-footer"><span id="quick-counter" role="status"></span><button type="button" class="btn primary" data-action="review">${esc(t('review'))} →</button></div>`;
    updateQuickCount();
  }
  function renderNav() {
  ...
  function itemDetails(i) { const v = state.items[i.id]; return [v.quantity?`${t('quantity')}: ${v.quantity}`:'',v.owner&&v.owner!=='tbd'?`${t('owner')}: ${t(v.owner)}`:'',v.status&&v.status!=='discuss'?`${t('status')}: ${t(v.status)}`:''].filter(Boolean).join(' · '); }
  function briefHTML(editable=false) {
    ...
    const simple = chosenEvents().filter(s=>!selected(s).length&&!META_FIELDS.some(k=>sectionState(s)[k]));
    if(simple.length) result += `<section class="review-section"><h3>${esc(t('yourEvents'))}</h3><ul class="brief-shortlist">${simple.map(s=>`<li>${esc(tr(s.title))}</li>`).join('')}</ul></section>`;
    if(state.services.length) result += `<section class="review-section"><h3>${esc(t('sharedServices'))}</h3><ul class="brief-shortlist">${state.services.map(id=>`<li>${esc(t(id))}</li>`).join('')}</ul></section>`;
    DATA.forEach((s,index) => {
      if(simple.includes(s)) return;
    ...
    const skipped = state.view==='quick' ? [] : DATA.filter(s=>inclusion(s)==='skip');
  ...
  function briefText() {
    ...
    if(state.services.length) lines.push('',t('sharedServices'),...state.services.map(id=>`• ${t(id)}`));
    DATA.forEach(s=>{
      if (!relevant(s)) return;
    ...
    if(state.view!=='quick') lines.push('',t('excluded')+': '+DATA.filter(s=>inclusion(s)==='skip').map(s=>tr(s.title)).join(', '));
  ...
  function renderReview() {
    ...
    if(state.view==='quick') {
      $('.review-stats').innerHTML=`<div><strong>${chosenEvents().length}</strong><span>${esc(t('eventsCount'))}</span></div><div><strong>${state.services.length}</strong><span>${esc(t('servicesCount'))}</span></div>`;
      const empty=$('.review-empty');
      if(empty) empty.innerHTML=`<h3>${esc(t('emptyTitle'))}</h3><p>${esc(t('quickEmpty'))}</p>`;
    }
  ...
  function renderAll() {
    ...
    document.querySelectorAll('button[data-view]').forEach(el=>el.setAttribute('aria-pressed',String(el.dataset.view===state.view)));
  ...
  function renderContent() {
    $('#client-details').hidden=reviewing;
    if(reviewing)renderReview();else if(state.view==='quick')renderQuick();else renderStep();
  }
  function navigate(index,isReview=false) {
    active = Math.max(0,Math.min(DATA.length-1,index)); reviewing = isReview;
    if(!isReview&&state.view==='quick'){state.view='detailed';renderAll();}
  ...
  function updateInput(el) {
    if(el.dataset.event) {
      ...
      state.sections[id]||={};
      state.sections[id].inclusion=el.checked?'include':'skip';
      save();renderNav();updateQuickCount();
    }
    if(el.dataset.service) {
      state.services=SERVICES.filter(id=>id===el.dataset.service?el.checked:state.services.includes(id));
      save();updateQuickCount();
    }
  ...
  document.addEventListener('click',async e=>{
    ...
    if(view){state.view=view.dataset.view;reviewing=false;$('#client-details').open=state.view==='quick';renderAll();return;}
