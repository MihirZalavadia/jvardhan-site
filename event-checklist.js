...
(() => {
  'use strict';
  const DATA = window.JV_LEGACY_CHECKLIST.sections;
  const KEY = 'jvardhan-event-checklist-v1';
  const PREVIOUS_KEY = 'jvardhan-wedding-checklist-v1';
  ...
  Object.assign(COPY, {
    eventCount:['૧૮૧ વસ્તુઓ · ૧૦ વિભાગો','181 items · 10 sections'],
    masterChecklist:['માસ્ટર ચેકલિસ્ટ · ૫૦ સેવાઓ →','Master checklist · 50 services →'],
    localNotice:['આ ચેકલિસ્ટનો ડ્રાફ્ટ આ બ્રાઉઝરમાં અલગથી સચવાય છે. આયોજકને મોકલવા માટે સારાંશ ડાઉનલોડ અથવા શેર કરો.','This checklist has its own draft in this browser. Download or share the brief to send it to your planner.'],
    ...
    yourEvents:['તમારા પ્રસંગો','Your events'],
  ...
  function savedItemNote(old,lang) {
    ...
    const label = key => COPY[key][lang==='gu'?0:1];
    const quantity = cleanText(old.quantity,80);
    return [quantity?`${label('quantity')}: ${quantity}`:'',
      OWNERS.includes(old.owner)&&old.owner!=='tbd'?`${label('owner')}: ${label(old.owner)}`:'',
      STATUSES.includes(old.status)&&old.status!=='discuss'?`${label('status')}: ${label(old.status)}`:'',
  ...
  try {
    ...
    if (saved !== null) state = sanitize(JSON.parse(saved));
    else {
      // Recover the pre-master answers once. Never write to the master page's key.
      const previous = JSON.parse(localStorage.getItem(PREVIOUS_KEY) || 'null');
      if (previous?.version === 1) state = sanitize(previous);
      else if (previous?.version === 2 && previous.legacy) {
        state = sanitize({version:1,lang:previous.lang,client:previous.client,services:previous.services,
          sections:previous.legacy.sections,items:previous.legacy.items});
      }
    }
  ...
  function renderAll() {
    ...
    document.title = state.lang==='gu'?'પ્રસંગવાર લગ્ન ચેકલિસ્ટ | J.Vardhan':'Event Wedding Checklist | J.Vardhan';
    ...
    $('#alternate-checklist').href = `checklist.html?lang=${state.lang}`;
  ...
  function downloadBrief() { download('\uFEFF'+briefText(),`J-Vardhan-event-brief-${state.lang}.txt`,'text/plain;charset=utf-8'); }
  ...
  document.addEventListener('click',async e=>{
    ...
    if(action==='export'){download(JSON.stringify(state,null,2),'J-Vardhan-event-checklist-draft.json','application/json');toast(t('downloaded'));}
    ...
    if(action==='share'){
      try{
        ...
        const file=new File(['\uFEFF'+text],`J-Vardhan-event-brief-${state.lang}.txt`,{type:'text/plain'});
